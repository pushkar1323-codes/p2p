#!/usr/bin/env bash
#
# Deploys the eligibility_registry Soroban contract to Stellar Testnet.
#
# WHY THIS EXISTS: loan_registry's create_loan_request now requires a
# configured eligibility dependency contract (L3-P07) — without one,
# every create_loan_request call fails with
# EligibilityContractNotConfigured. eligibility_registry must be
# deployed and initialized BEFORE loan_registry is (re)deployed and
# wired to it via set_eligibility_contract (see
# init_loan_registry_testnet.sh).
#
# WHAT THIS DOES, IN ORDER (mirrors deploy_testnet.sh exactly, just
...
#      deployment to contracts/eligibility_registry/DEPLOYMENTS.md.
#
# This script only deploys the contract. It does NOT call initialize
# or set_eligibility — run init_eligibility_testnet.sh next, then
# allowlist_borrower_testnet.sh for each borrower wallet that should
# be able to create loan requests.
#
# PREREQUISITES (see contracts/.env.example for config):
...
#
# USAGE:
#   cd contracts && ./scripts/deploy_eligibility_testnet.sh
#
# After it prints a contract ID, copy that value into
# ELIGIBILITY_REGISTRY_CONTRACT_ID in contracts/.env before running
# init_eligibility_testnet.sh.
#
...
CRATE_DIR="$CONTRACTS_DIR/eligibility_registry"
WASM_PATH="$CONTRACTS_DIR/target/wasm32-unknown-unknown/release/eligibility_registry.wasm"
...
echo "Building eligibility_registry for wasm32-unknown-unknown (release)..."
( cd "$CONTRACTS_DIR" && cargo build --locked --release --target wasm32-unknown-unknown -p eligibility_registry )
...
echo ""
echo "Next: add this to contracts/.env as ELIGIBILITY_REGISTRY_CONTRACT_ID,"
echo "then run ./scripts/init_eligibility_testnet.sh."
...

{
  echo ""
  echo "## $DEPLOYED_AT"
  echo "- Contract: eligibility_registry"
  echo "- Network: Stellar Testnet"
  echo "- Contract ID: \`$CONTRACT_ID\`"
  echo "- Deployer address: \`$DEPLOYER_ADDRESS\`"
  echo "- WASM artifact: \`target/wasm32-unknown-unknown/release/eligibility_registry.wasm\`"
  echo "- Deployed via: \`contracts/scripts/deploy_eligibility_testnet.sh\`"
} >> "$DEPLOYMENTS_RECORD"
